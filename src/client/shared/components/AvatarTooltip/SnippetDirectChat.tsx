...
import { INITIAL_SUBGROUPING_KEYS } from '../../store/constants'
...
const SnippetDirectChat: React.FC<{
  ...
  const loggedInUserLangIds: string[] =
    userLangsSubGroupings[`${LOGGED_IN_USER_SUBGROUPING_KEY}${loggedInUserId}`]
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <Table style={{ fontSize: '.95rem', margin: '5px 0 11px' }} size="small">
        ...
        <TableBody>
          {userLangsSubGroupings[`${LOGGED_IN_USER_SUBGROUPING_KEY}${id}`].map(
            (id) => {
